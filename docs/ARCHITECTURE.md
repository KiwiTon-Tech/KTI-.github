...
> **Status**: In progress (Phase 5 complete — all services wired into Gateway, dashboard aggregation live)
...
| 5 | **Slim `KTI-Strategy-Engine`** down to strategies + orchestrator. Slim `Kiwiton-Investments-Backend` into `KTI-Gateway`. | ✅ **Gateway Phase 5 complete (2026-05-19).** All downstream services proxied: `/broker/*` → KTI-Broker-Service, `/market/*` → KTI-Market-Data-Service, `/news/*` → KTI-News-Sentiment-Service, `/ml/*` → KTI-ML-Service, `/backtest/*` → KTI-Backtest-Service. Dashboard aggregation endpoint `GET /dashboard/` fetches account + positions + recent orders + per-symbol sentiment + service health probes in parallel (ThreadPoolExecutor, per-service fault isolation). Google OAuth → JWT flow live. `@require_auth` enforced on all protected routes. Rate limiting + CSRF protection active. `Kiwiton-Investments-Backend` archived to `_archive/`. **Phase 5b (pending):** wire `KTI-Strategy-Engine` orchestrator routes (`/orchestrator/*`), add `kti-db` dependency for auth/session tables + trade history reads. |
